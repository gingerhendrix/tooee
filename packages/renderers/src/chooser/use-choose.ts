...
export interface UseChooseOptions {
  ...
  /** Whether Escape leaves insert mode before cancelling (default true). */
  escapeToCursor?: boolean;
...
export const useChoose = function useChoose(options: UseChooseOptions): UseChooseResult {
  ...
  useEffect(() => {
    optionsRef.current.onActiveChange?.(activeItem);
  }, [activeItem, optionsRef]);
  ...
  const builtInActions = useMemo<ActionDefinition[]>(() => {
    const definitions: ChooseKeymapDefinition[] = [
      {
        group: () => (escapeToCursor && modeRef.current === "insert" ? "mode" : "cancel"),
        handler: () => {
          if (escapeToCursor && modeRef.current === "insert") {
            setMode("cursor");
          } else {
            cancel();
          }
        ...
        title: "Back / cancel",
        when: () =>
          escapeToCursor && modeRef.current === "insert"
            ? true
            : optionsRef.current.onCancel !== undefined,
      },
      {
        group: () => "cancel",
        handler: cancel,
