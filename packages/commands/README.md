...
- **`panel`** — a _peer_ surface that owns input only while it is its group's active panel (see [`@tooee/panels`](../panels)). Selection is by activation, not stack depth/order. Unlike a modal, an active panel does **not** swallow: keys it neither matches nor holds a pending chord for **fall through** to the enclosing surface.
